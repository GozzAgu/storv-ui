...
export function receiptBalanceDue(
  receipt: Pick<Receipt, 'total' | 'amountPaid' | 'balanceDue' | 'payments'>
): number {
    ...
    const row: {
      ...
      paidAt:
        p.paidAt instanceof Date
          ? p.paidAt
          : p.paidAt &&
            typeof p.paidAt === 'object' &&
            'toDate' in p.paidAt &&
            typeof p.paidAt.toDate === 'function'
          ? p.paidAt.toDate()
