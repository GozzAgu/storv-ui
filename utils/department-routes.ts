...
export function isDepartmentsAreaPath(path: string): boolean {
  ...
  if (normalized.startsWith('/dashboard/departments')) return true
  return /^\/dashboard\/stores\/[^/]+\/departments(?:\/|$)/.test(normalized)
}
