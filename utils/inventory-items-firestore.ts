...
import { query, where, orderBy, limit, startAfter, getDocs, type CollectionReference } from 'firebase/firestore'
...
export async function getInventoryItemsPage(
  ...
  for (let p = 1; p <= page; p++) {
    const snap = await getOnePageOrdered(itemsRef, folderId, queryUserId, isStaff, pageSize, p === 1 ? null : cursor)
    lastPageItems = snap.docs.map((doc) => docToInventoryItem(doc, folderId, queryUserId))
    sortItemsDesc(lastPageItems)
    pageDataCache.set(pageCacheKey(folderId, p, pageSize), { items: lastPageItems, fetchedAt: Date.now() })
...
export async function fetchAllInventoryItemsChunked(
  ...
  try {
    for (;;) {
      const snap = await getOnePageOrdered(itemsRef, folderId, queryUserId, isStaff, pageSize, cursor)
      if (snap.empty) break
    ...
    const snap = await getDocs(buildUnorderedPageQuery(itemsRef, folderId, queryUserId, isStaff, 5000))
    for (const doc of snap.docs) {
      all.push(docToInventoryItem(doc, folderId, queryUserId))
    }
  }
