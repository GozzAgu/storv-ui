...
function finalizeFieldOrder(
  ...
  const name = columnOrderFields.find((f) => f.name === 'name') ?? defaultNameField(ts)
  const price = columnOrderFields.find((f) => f.name === 'price') ?? defaultPriceField(ts)
  const serial = columnOrderFields.find((f) => f.name === 'serialNo' || f.name === 'serialNumber')
  const brand = columnOrderFields.find((f) => f.name === 'brand')

  const lockedNames = new Set(['name', 'price', 'serialNo', 'serialNumber', 'brand', 'model'])
...
export function parseTemplateFieldsFromExcelArrayBuffer(
  ...
  if (nonEmpty.length === 0) {
    throw new Error('No column headers found in row 1. Put titles in the first row of the sheet.')
  }
