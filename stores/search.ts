...
function inventoryItemToSearchResult(
  item: InventoryItem,
  folderName: string,
  folderId: string
): SearchResult {
    ...
    async performSearch() {
      ...
      try {
          ...
          receiptsStore.receipts.forEach((receipt) => {
            ...
            const matchesDateRange =
              ...
              (() => {
                ...
                return (
                  receiptDate >= this.filters.dateRange!.start! &&
                  receiptDate <= this.filters.dateRange!.end!
                )
              })()
            ...
            if (matchesQuery && matchesDateRange && matchesStatus) {
              results.push({
                ...
                description: `${receipt.itemsCount} items • $${receipt.total.toFixed(2)} • ${
                  receipt.status
                }`,
        ...
        if (entityTypes.includes('customers')) {
          ...
          if (customersStore.customers.length === 0) {
            console.warn(
              '[SearchStore] No customers found in store. This might be expected if no customers exist yet.'
            )
          }
          ...
          customersStore.customers.forEach((customer) => {
            ...
            const matchesDateRange =
              ...
              (() => {
                ...
                return (
                  lastOrderDate >= this.filters.dateRange!.start! &&
                  lastOrderDate <= this.filters.dateRange!.end!
                )
              })()
            ...
            if (matchesQuery && matchesDateRange) {
              results.push({
                ...
                description: `${customer.totalOrders} orders • $${customer.totalSpent.toFixed(
                  2
                )} total`,
                icon: 'UserCircleIcon',
        ...
        results.sort((a, b) => {
          const aExact =
            a.title.toLowerCase() === searchQuery || a.subtitle.toLowerCase() === searchQuery
          const bExact =
            b.title.toLowerCase() === searchQuery || b.subtitle.toLowerCase() === searchQuery
      ...
      try {
        ...
        try {
          ...
          if (
            ...
            const q = query(savedSearchesRef, where('createdBy', '==', userId))
            querySnapshot = await getDocs(q)
          } else {
            throw orderByError
          }
