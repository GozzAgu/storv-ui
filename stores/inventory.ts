  ...
  actions: {
      ...
      try {
        ...
        try {
          ...
          if (userStore.userData?.role === 'staff') {
            ...
            q = query(foldersRef, orderBy('createdAt', 'desc'))
          } else {
            // Superadmin sees only their folders
            q = query(foldersRef, where('createdBy', '==', userId), orderBy('createdAt', 'desc'))
          }
          ...
          if (
            ...
            if (userStore.userData?.role === 'staff') {
              ...
              // Superadmin sees only their folders
              q = query(foldersRef, where('createdBy', '==', userId))
            }
            querySnapshot = await getDocs(q)
          } else {
            throw orderByError
          ...
          try {
            ...
            if (staffDepartmentId) {
              ...
              folders = folders.filter(
                (folder) => !folder.allowedDepartments || folder.allowedDepartments.length === 0
              )
            }
          } catch (error: any) {
            console.warn(
              '[InventoryStore] Could not filter by department, showing all folders:',
              error.message
            )
          }
        }
    ...
    async duplicateFolderTemplatesBetweenStores(
      ...
      for (const src of foldersToCopy) {
        ...
        const tpl = src.template ? stripUndefinedDeep(clonePlain(src.template)) : undefined
    ...
    async createFolder(
      ...
      if (userStore.userData?.role === 'staff') {
        ...
        throw new Error(
          'Managers cannot create inventory folders. Only super admins can create folders.'
        )
      }
    ...
    async updateFolder(
      folderId: string,
      updates: Partial<Omit<InventoryFolder, 'id' | 'createdAt' | 'createdBy'>>
    ) {
      const db = useFirestore().getFirestoreInstance()
      if (!db) {
        throw new Error(CLOUD_UNAVAILABLE_MESSAGE)
      ...
      if (userStore.userData?.role === 'staff') {
        ...
        if (currentStaffMember?.role !== 'manager') {
          throw new Error(
            'Staff members do not have permission to update folders. Only managers can edit.'
          )
        }
      }
    ...
    async deleteFolder(folderId: string) {
      ...
      if (userStore.userData?.role === 'staff') {
        ...
        if (currentStaffMember?.role !== 'manager') {
          throw new Error(
            'Staff members do not have permission to delete folders. Only managers can delete.'
          )
        }
      }
    ...
    async _prepareItemsQueryContext(folderId: string) {
      ...
      if (userStore.userData?.role === 'staff') {
        ...
        try {
            ...
            if (folder) {
              if (folder.allowedDepartments && folder.allowedDepartments.length > 0) {
                if (!folder.allowedDepartments.includes(staffDepartmentId)) {
                  throw new Error(
                    'Access denied: Your department does not have access to items in this folder'
                  )
                }
              }
            } else {
          ...
          if (e?.message?.includes('Access denied') || e?.message?.includes('Folder not found'))
            throw e
    ...
    async checkDuplicateSerialNumber(
      ...
      try {
        ...
        return await duplicateSerialExistsViaApi(() => authStore.currentUser!.getIdToken(), {
        ...
        throw new Error(
          error?.data?.message ||
            error.message ||
            'Could not validate duplicate serial number. Please try again.'
        )
      }
    },
    ...
    async createItem(
      folderId: string,
      itemData: Omit<InventoryItem, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'folderId'>
    ) {
      const { isDemoModeActive } = await import('~/utils/demo-mode')
      if (isDemoModeActive()) {
      ...
      try {
        ...
        this.updateItemCount(folderId).catch((err) => {
          console.warn(
            '[InventoryStore] Background folder stats update failed (non-critical):',
            err
          )
    ...
    async createItemsBatch(
      folderId: string,
      itemsData: Array<
        Omit<InventoryItem, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'folderId'>
      >
    ) {
      const { isDemoModeActive } = await import('~/utils/demo-mode')
      if (isDemoModeActive()) {
      ...
      try {
        ...
        const folderIndex = this.folders.findIndex((f) => f.id === folderId)
        if (folderIndex > -1 && this.folders[folderIndex]) {
          this.folders[folderIndex].itemCount =
            (this.folders[folderIndex].itemCount || 0) + itemsData.length
        }
    ...
    async updateItem(
      folderId: string,
      itemId: string,
      updates: Partial<Omit<InventoryItem, 'id' | 'createdAt' | 'createdBy' | 'folderId'>>
    ) {
      const { isDemoModeActive } = await import('~/utils/demo-mode')
      if (isDemoModeActive()) {
    ...
    async updateLowStockCount(folderId: string) {
      ...
      try {
        ...
        const lowStockThreshold =
          userData?.storeDetails?.settings?.inventory?.lowStockThreshold || 10
      ...
      try {
        ...
        for (const [loanId, soldIds] of soldByLoan) {
          ...
          const rawLines = Array.isArray(data.lines)
            ? (data.lines as Array<{ inventoryItemId: string }>)
            : []
      ...
      try {
        if (useBulkRestore && options.folderId && options.restoreQuantities) {
          ...
          await logActivity({
            ...
            entityName: `Stock restored for ${
              Object.keys(options.restoreQuantities).length
            } product line(s)`,
            storeId,
            userId: authStore.currentUser!.uid,
    ...
    async applyBulkDiscount(
      ...
      try {
        ...
        const batch = updates.map(
          ({ itemId, discountPercentage, discountAmount, originalPrice, discountedPrice }) => {
            // Use hierarchical path: users/{userId}/stores/{storeId}/inventoryItems/{itemId}
            const itemRef = getInventoryItemDocument(db, userId, storeId, itemId)
            return updateDoc(itemRef, {
              discountPercentage,
        ...
        if (storeIdBulk) {
          ...
          await logActivity({
            ...
            entityType: 'items_batch',
            entityId: folderId,
            entityName: `Discount applied to ${updates.length} item${
              updates.length !== 1 ? 's' : ''
            }`,
            storeId: storeIdBulk,
            userId: authStore.currentUser!.uid,
    ...
    async removeBulkDiscount(folderId: string, itemIds: string[]) {
      ...
      try {
        ...
        await logActivity({
          ...
          entityType: 'items_batch',
          entityId: folderId,
          entityName: `Discount removed from ${itemIds.length} item${
            itemIds.length !== 1 ? 's' : ''
          }`,
          storeId,
          userId: authStore.currentUser!.uid,
