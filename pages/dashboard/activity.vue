<template>
  <div>
    <div class="mb-6 sm:mb-8">
      ...
      <h1 class="mt-1 text-lg sm:text-xl font-semibold text-gray-900 dark:text-gray-100 tracking-tight">Activity Logs</h1>
    ...
    <div v-if="!canAccess" class="rounded-2xl ring-1 p-6 text-center max-w-md mx-auto" :class="accessDeniedByRole ? 'bg-red-50 dark:bg-red-900/20 ring-red-200/60 dark:ring-red-700/50' : 'bg-amber-50 dark:bg-amber-900/20 ring-amber-200/60 dark:ring-amber-700/50'">
      <p v-if="accessDeniedByRole" class="text-sm text-red-800 dark:text-red-200">Activity Logs are only available to super admins and managers.</p>
      <p v-else class="text-sm text-amber-800 dark:text-amber-200">Activity Logs are available on Storvv Medium and Storvv Enterprise. Upgrade your plan to view logs.</p>
      <NuxtLink v-if="!accessDeniedByRole" to="/dashboard/settings" class="mt-3 inline-block text-sm font-medium text-primary-600 dark:text-primary-400 hover:text-primary-500">Go to Settings →</NuxtLink>
    </div>
    ...
    <div v-if="!storeId" class="rounded-2xl bg-amber-50 dark:bg-amber-900/20 ring-1 ring-amber-200/60 dark:ring-amber-700/50 p-6 text-center">
      <p class="text-sm text-amber-800 dark:text-amber-200">Select a store to view activity logs.</p>
    </div>
    ...
    <div v-else class="rounded-2xl bg-white dark:bg-gray-800/90 shadow-sm ring-1 ring-gray-200/60 dark:ring-gray-700/50 overflow-hidden">
      ...
      <div v-else-if="logs.length === 0" class="p-12 text-center">
        <p class="text-sm text-gray-500 dark:text-gray-400">No activity recorded yet for this store.</p>
        <p class="mt-1 text-xs text-gray-400 dark:text-gray-500">Create or edit inventory items to see logs here.</p>
      </div>
      <div v-else class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200/80 dark:divide-gray-700/80">
          <thead class="bg-gray-50 dark:bg-gray-800/50">
            <tr>
              <th scope="col" class="px-4 sm:px-5 py-3 text-left text-[10px] font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400">
                User
              </th>
              <th scope="col" class="px-4 sm:px-5 py-3 text-left text-[10px] font-bold uppercase tracking-wider text-gray-600 dark:text-gray-400">
                Action
          ...
          <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200/80 dark:divide-gray-700/80">
            ...
            >
              <td class="px-4 sm:px-5 py-2.5 text-xs font-medium text-gray-900 dark:text-gray-100 whitespace-nowrap">
                {{ log.userDisplayName }}
              </td>
              <td class="px-4 sm:px-5 py-2.5 whitespace-nowrap">
                <span
                  :class="[
                    'inline-flex px-2 py-0.5 text-[10px] font-medium capitalize rounded-full',
                    log.action === 'created'
              ...
              <td class="px-4 sm:px-5 py-2.5 text-xs text-gray-700 dark:text-gray-300 max-w-[200px] truncate" :title="log.entityName">
                {{ log.entityName }}
              </td>
              <td class="px-4 sm:px-5 py-2.5 text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap">
