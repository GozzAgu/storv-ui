...
watch(searchQuery, () => {
  ...
  if (!trimmed) {
    ...
    if (!isLoadingFolder.value && fid) {
      inventoryStore
        .fetchItemsPage(fid, currentPage.value, itemsPerPage.value, { force: true })
        .catch((err) => {
...
const startInlineEdit = async (item: InventoryItem, column: { key: string; type?: string }) => {
  if (!canManageInventoryItems.value || isInventoryItemLocked(item) || !isColumnEditable(column))
    return
  if (!isLargeScreen.value) return
  let val = item[column.key]
  // For price columns with discount, show the effective (discounted) price
  if (
    (column.type === 'currency' || column.key.toLowerCase().includes('price')) &&
    item.discountedPrice !== undefined
  ) {
  ...
  if (val !== undefined && val !== null) {
    if (column.key === 'dateIn' || column.key === 'dateOut' || column.type === 'date') {
      const d =
        val && typeof val === 'object' && 'toISOString' in val
          ? (val as Date).toISOString().split('T')[0] ?? ''
...
const saveInlineEdit = async () => {
  ...
  const prevDateStr =
    previousValue && typeof previousValue === 'object' && 'toISOString' in previousValue
      ? (previousValue as Date).toISOString().split('T')[0]
      : typeof previousValue === 'string' && previousValue.includes('T')
      ? previousValue.split('T')[0]
      : previousValue
  ...
  const hasChanged =
    isPriceField && item.discountedPrice !== undefined
...
const effectiveTemplateFields = computed((): TemplateField[] => {
  ...
  if (folder.value?.hasSerialNumbers) {
    return fields.filter(
      (f) => !STOCK_LIKE_NAMES.includes(f.name.toLowerCase() as (typeof STOCK_LIKE_NAMES)[number])
    )
  }
  ...
  if (fieldSource.length > 0) {
    const mapped = fieldSource
      ...
      .map((field) => ({
        ...
        sortable: true,
        type:
          field.type === 'currency' || field.name.toLowerCase() === 'price'
            ? 'currency'
            : field.type,
      }))
...
const createSellerLoanModalItems = computed(
  (): InventoryItem[] =>
    sellerLoanModalItemsOverride.value ?? selectedItemsEligibleForSellerLoan.value
)
...
const getItemAvailability = (item: InventoryItem) => {
  ...
  const refundedReceipts = receiptsStore.receipts.filter(
    (r) => r.status === 'refunded' && r.itemIds?.includes(item.id)
  )
  ...
  if (isItemAwaitingPayment(item)) {
    ...
    const receipt = receiptId ? receiptsStore.receipts.find((r) => r.id === receiptId) : undefined
    ...
    if (typeof date === 'string' && date.includes('Timestamp') && date.includes('seconds=')) {
      try {
        ...
        if (secondsMatch && secondsMatch[1]) {
          ...
          const nanoseconds =
            nanosecondsMatch && nanosecondsMatch[1] ? parseInt(nanosecondsMatch[1], 10) : 0
...
const handleEditItem = (item: InventoryItem) => {
  ...
  if (
    ...
    (itemForm[qk] === undefined ||
      itemForm[qk] === null ||
      itemForm[qk] === '' ||
      Number(itemForm[qk]) < 1)
  ) {
    itemForm[qk] = 1
  }
  showAddItemModal.value = true
...
const handleConfirmDuplicate = async () => {
  ...
  if (trimmedInputs.length !== serials.length) {
    toast.error(
      'Duplicate serial numbers are not allowed. Please ensure each serial number is unique.'
    )
    return
  }
  ...
  if (alreadyExist.length > 0) {
    toast.error(
      alreadyExist.length === 1
        ...
        : `These serial numbers for ${productLabel} already exist: ${alreadyExist.join(
            ', '
  ...
  try {
    if (editingItem.value) {
      ...
      if (folder.value?.hasSerialNumbers && serialNumbers.value.length > 0) {
        ...
        if (alreadyExist.length > 0) {
          toast.error(
            alreadyExist.length === 1
              ...
              : `These serial numbers for ${productLabel} already exist in this folder: ${alreadyExist.join(
                  ', '
        ...
        toast.success(
          `Successfully created ${validSerialNumbers.length} product${
            validSerialNumbers.length !== 1 ? 's' : ''
          }`
        )
      } else {
  ...
  try {
    ...
    headers.forEach((header, index) => {
      const field = templateFields.find(
        (f) =>
          f.label?.toLowerCase() === header?.toString().toLowerCase() ||
    ...
    if (missingRequiredFields.length > 0) {
      toast.error(
        `Missing required columns: ${missingRequiredFields
    ...
    for (let i = 1; i < jsonData.length; i++) {
      ...
      templateFields.forEach((field) => {
        ...
        const headerIndex = headers.findIndex(
          (h) =>
            field.label?.toLowerCase() === h?.toString().toLowerCase() ||
        ...
        try {
          switch (field.type) {
            ...
            case 'select':
              if (field.options && !field.options.includes(value)) {
                errors.push(
                  `Row ${i + 1}: Invalid option "${value}" for "${
                    field.label || field.name
          ...
          errors.push(
            `Row ${i + 1}: Error parsing "${field.label || field.name}": ${error.message}`
          )
          hasError = true
        }
      })
    ...
    if (itemsToImport.length > 0) {
      ...
      if (serialNoField || hasSerialNumberData) {
        ...
        let existingItems = await inventoryStore.fetchItemsAllChunked(currentFolderId, {
          force: true,
        })
        ...
        existingItems.forEach((item) => {
          ...
          if (item.folderId !== currentFolderId) {
            console.error(
              '[Import] ERROR: Item has wrong folderId!',
              item.id,
              item.folderId,
              'expected:',
              currentFolderId
        ...
        itemsToImport.forEach((itemEntry) => {
          ...
          const serialNo =
            (serialNoField ? itemData[serialNoField.name] : null) ||
            itemData['serialNo'] ||
            itemData['serialNumber'] ||
            itemData['serialno'] ||
            itemData['serialnumber'] ||
    ...
    if (itemsToImport.length === 0) {
      ...
      if (reasons.length > 0) {
        toast.warning(
          `No new items to import. All items were skipped: ${reasons.join(', ')}.`,
          5000
        )
      } else {
    ...
    if (errors.length > 0) {
      ...
      toast.warning(
        `Found ${errorCount} validation error(s) that will be skipped${moreErrors}. The import will continue with valid items.`,
        6000
      )
    }
    ...
    for (const itemEntry of itemsToImport) {
      try {
        ...
        errors.push(
          `Row ${itemEntry.rowNumber}: Failed to create product - ${
