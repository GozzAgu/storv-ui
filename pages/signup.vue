      ...
      <div class="bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl p-8 sm:p-10 border border-white/50">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          >
            ...
            <span class="ml-2">{{ isGoogleLoading ? 'Signing up...' : 'Google' }}</span>
          </button>
          <button
            type="button"
...
const handleGoogleSignUp = async () => {
  ...
  try {
    ...
    if (user) {
      ...
      // If user doesn't exist in Firestore, create user document (new signup)
      if (!userData) {
...
useHead({
  ...
  meta: [
    {
      name: 'description',
      content: 'Create your Storv account'
    }
  ]
})
</script>
