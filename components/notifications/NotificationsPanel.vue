<template>
  <div
    :class="[
      'rounded-sm bg-white shadow-[0_12px_30px_-18px_rgba(15,23,42,0.3)] dark:bg-dashboard-card dark:shadow-[0_12px_28px_-16px_rgba(0,0,0,0.55)] overflow-hidden flex flex-col',
      variant === 'dropdown' ? 'max-h-[min(85vh,24rem)] w-full min-w-[18rem]' : '',
    ]"
  >
    <!-- Header: title + Mark all as read -->
    <div
      class="flex items-center justify-between px-3.5 pt-3 pb-2 border-b border-gray-100 shrink-0 bg-gray-50/60 dark:bg-white/[0.02]"
    >
    ...
    <div
      class="flex items-center gap-0.5 border-b border-gray-100 px-3.5 shrink-0 bg-white/70 dark:bg-dashboard-card"
    >
      <button
        type="button"
        :class="[
          'relative flex items-center gap-1 rounded-[2px] px-2.5 py-2 text-[11px] font-medium transition-colors border-b-2 -mb-px',
          activeTab === 'inbox'
            ? 'text-primary-600 dark:text-primary-400 border-primary-500 dark:border-primary-400 bg-primary-50/50 dark:bg-primary-500/10'
            : 'text-gray-500 dark:text-gray-400 border-transparent hover:text-gray-700 dark:hover:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/[0.03]',
        ]"
    ...
    <div
      :class="[
        ...
        variant === 'dropdown' ? 'overscroll-contain' : '',
      ]"
    >
      <!-- Loading State -->
      <div
      ...
      >
        <div
          class="w-6 h-6 rounded-full bg-gray-100 dark:bg-white/10 flex items-center justify-center mb-1.5"
        >
      ...
      <div v-else class="divide-y divide-gray-100 dark:divide-gray-700/80">
        ...
        >
          <div
            :class="[
              'relative shrink-0 w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-semibold text-white leading-none',
          ...
          <div class="flex-1 min-w-0 pr-1.5">
            <p class="text-[11px] text-gray-900 dark:text-gray-100 leading-snug">
              ...
              <span class="font-normal text-gray-600 dark:text-gray-400">
                - {{ formatMessageWithAccountCurrency(notification.message) }}</span
              >
            </p>
      ...
      <div
        ...
        class="border-t border-gray-100 px-3.5 py-2 flex justify-center shrink-0 bg-gray-50/50 dark:bg-white/[0.02]"
      >
        <button
          type="button"
...
const avatarColors = ['bg-blue-500', 'bg-blue-600', 'bg-gray-500', 'bg-gray-600'] as const
...
function getAvatarBgClass(notification: Notification): string {
  const type = notification.type
  const index = type
    ? Math.abs(type.split('').reduce((a, c) => a + c.charCodeAt(0), 0)) % avatarColors.length
    : 0
