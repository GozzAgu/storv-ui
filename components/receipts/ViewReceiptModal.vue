      ...
      >
        ...
        <div v-if="receipt.isSwapIn" class="px-6 pb-3">
          ...
          <template v-else-if="swapInItemError">
            <p class="text-[12px] leading-snug text-amber-800 dark:text-amber-300/90">{{ swapInItemError }}</p>
          </template>
          <template v-else-if="swapInPrimaryLabel">
            <p class="text-[12px] font-medium leading-snug text-gray-900">{{ swapInPrimaryLabel }}</p>
          ...
          <template v-else-if="receipt.swapInItemId">
            <p class="text-[12px] leading-snug text-gray-500">Trade-in recorded; details unavailable.</p>
          </template>
        </div>
...
watch(
  () =>
    [props.modelValue, props.receipt?.isSwapIn, props.receipt?.swapInItemId, props.receipt?.storeId] as const,
