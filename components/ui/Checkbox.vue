<template>
  ...
  >
    ...
    <div
      :class="[
        'relative flex items-center justify-center rounded-sm border-0 transition-all duration-200 ease-out',
        'transform',
        size === 'sm' ? 'w-4 h-4' : size === 'lg' ? 'w-6 h-6' : 'w-5 h-5',
        modelValue
          ? 'bg-gradient-to-br from-primary-400 to-primary-500 scale-100'
          : 'bg-gray-100 dark:bg-gray-700 group-hover:bg-gray-200 dark:group-hover:bg-gray-600 scale-100',
    ...
    <span
      ...
      :class="[
        ...
        modelValue ? 'text-gray-900 dark:text-gray-100' : 'text-gray-700 dark:text-gray-300',
        labelClass,
      ]"
    >
      <slot>{{ label }}</slot>
