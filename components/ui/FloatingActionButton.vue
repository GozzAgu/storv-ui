...
<template>
  <div :class="['fixed z-50', positionClasses]">
    ...
    >
      <div v-if="isOpen && actions && actions.length > 0" class="mb-3 space-y-2">
        <button
          v-for="(action, index) in actions"
          :key="action.id"
          @click="handleAction(action)"
          :class="[
            'flex items-center gap-3 px-4 py-3 rounded-sm transition-all transform hover:scale-105',
    ...
    <button
      @click="
        mainAction ? handleAction(mainAction) : actions && actions.length > 0 ? toggleMenu() : null
      "
      :class="[
        'w-14 h-14 rounded-full transition-all transform hover:scale-110 active:scale-95 flex items-center justify-center',
        mainAction
    ...
    >
      ...
      >
        <XMarkIcon
          v-if="isOpen && !mainAction && actions && actions.length > 0"
          key="close"
          class="w-6 h-6 text-white"
        />
        <component
          v-else-if="mainAction"
          :is="mainAction.icon"
          key="main"
          class="w-6 h-6 text-white"
        />
