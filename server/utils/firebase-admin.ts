...
function isServerlessRuntime(): boolean {
  return Boolean(
    process.env.VERCEL ||
      ...
      process.env.AWSLAMBDA_FUNCTION_NAME
  )
}
...
function resolveServiceAccountJsonString(config: { firebaseServiceAccount?: string }): string {
  ...
  const order = isServerlessRuntime()
    ? [fromEnv, fromNuxtEnv, fromConfig]
    : [fromConfig, fromNuxtEnv, fromEnv]
...
function getAdminApp(): App {
  ...
  try {
    ...
    if (fromJson) {
      ...
      const pathToRead = isAbsolute(credentialsPath)
        ? credentialsPath
...
export function getAdminStorageBucket() {
  ...
  const name = resolveFirebaseStorageBucket(pub.projectId || '', pub.storageBucket || '')
  if (!name) {
    throw new Error(
