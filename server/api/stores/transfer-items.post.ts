...
export default defineEventHandler(async (event) => {
  try {
    ...
    const { userId, sourceStoreId, destinationStoreId, folderId, destinationFolderId, items, notes } = body

    // Validate input
    if (!userId || !sourceStoreId || !destinationStoreId || !folderId || !destinationFolderId || !items || !Array.isArray(items) || items.length === 0) {
