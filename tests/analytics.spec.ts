...
test.describe('Analytics Page', () => {
  ...
  test('should display key metrics or store-selection state when authenticated', async ({
    page,
  }) => {
    if (await isLoginPage(page)) {
      test.skip()
    ...
    const visibleMetrics = await Promise.all(metrics.map((m) => m.isVisible().catch(() => false)))
