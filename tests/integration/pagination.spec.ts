...
describe('getVisiblePageNumbers', () => {
  ...
  it('shows window near end when current ≥ total − 2', () => {
    expect(getVisiblePageNumbers(8, 10)).toEqual([1, PAGINATION_ELLIPSIS, 7, 8, 9, 10])
    expect(getVisiblePageNumbers(9, 10)).toEqual([1, PAGINATION_ELLIPSIS, 7, 8, 9, 10])
    expect(getVisiblePageNumbers(10, 10)).toEqual([1, PAGINATION_ELLIPSIS, 7, 8, 9, 10])
  })
