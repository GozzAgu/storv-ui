...
export const useStaff = () => {
  ...
  const createStaff = async (
    staffData: Omit<Staff, 'id' | 'createdAt' | 'updatedAt' | 'createdBy' | 'departmentName'>
  ): Promise<string> => {
    const db = getFirestoreInstance()
    if (!db) {
      throw new Error('Firestore not initialized')
      ...
      if (updates.departmentId) {
        ...
        if (staffMember && staffMember.departmentId !== updates.departmentId) {
          ...
          await updateStaffCount(
            ...
            Math.max(0, (await getDepartmentStaffCount(staffMember.departmentId)) - 1)
          )
  ...
  const updateDepartmentManager = async (
    departmentId: string,
    managerName: string,
    managerId: string
  ) => {
