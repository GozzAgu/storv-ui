...
export function getReceiptProductDetails(
  item: InventoryItem
...
function keyLooksLikeMoneyField(key: string): boolean {
  ...
  if (k.endsWith('amount') && k.length <= 24 && !k.includes('qty') && !k.includes('quantity'))
    return true
  return false
}

function formatMoneyDetailSegment(
  key: string,
  rawText: string,
  opts?: ReceiptDetailLinesOptions
): string {
...
export function getProductDetailLines(
  item: ReceiptItem,
  opts?: ReceiptDetailLinesOptions
): string[] {
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    if (!text) continue
    const label =
      DETAIL_LABELS[key] || key.replace(/([A-Z])/g, ' $1').replace(/^./, (c) => c.toUpperCase())
    const normalized = `${label}:${text.toLowerCase()}`
    if (seen.has(normalized)) continue
    seen.add(normalized)
    lines.push(`${label}: ${formatMoneyDetailSegment(key, text, opts)}`)
...
export function getInventoryItemDetailLines(
  item: InventoryItem,
  opts?: ReceiptDetailLinesOptions
): string[] {
