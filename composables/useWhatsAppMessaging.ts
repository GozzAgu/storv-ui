...
export function useWhatsAppMessaging() {
 ...
 const data = (await $fetch(resolveApiPath('/api/whatsapp/usage'), { headers })) as WhatsAppUsageState
 usage.value = data
 } catch (e) {
 ...
 */
 ...
 if (planHasFeature(p, 'whatsapp_messaging') && getPlanLimits(p).maxWhatsAppMessagesPerMonth < 0) {
 return ''
 }
