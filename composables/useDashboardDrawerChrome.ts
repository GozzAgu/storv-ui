...
export function useDashboardDrawerChrome() {
  ...
  const pickListScrollClass =
    'max-h-[min(52vh,28rem)] overflow-y-auto overscroll-contain divide-y divide-gray-100/90 dark:divide-gray-800/80'

  const pickRowClass =
    'flex w-full items-center gap-3 px-3 py-2.5 text-left transition-colors hover:bg-gray-50/90 dark:hover:bg-white/[0.03]'

  const pickRowSelectedClass = 'bg-primary-50/80 ring-0 dark:bg-primary-950/25'
