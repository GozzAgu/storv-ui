 ...
 */
export function useItemTimeline(
 item: Ref<InventoryItem | null>,
 ...
 const soldQuantity =
 soldReceipt?.items?.find((ri) => ri.itemId === i.id)?.quantity ?? 1
 if (i.dateOut) {
 const dateOut = i.dateOut instanceof Date ? i.dateOut : new Date(i.dateOut)
 const unitsText =
 soldQuantity > 1 ? `${soldQuantity} units sold` : 'Item sold'
 events.push({
 type: 'sold',
 ...
 const restockHistory = (i as any).restockHistory as Array<{ date: Date | string; quantity: number }> | undefined
 ...
 const maintenanceHistory = (i as any).maintenanceHistory as Array<{ date: Date | string; note?: string }> | undefined
