...
export function useDashboardInsights(folderItems: Ref<Record<string, InventoryItem[]>>) {
 ...
 parts.push(
 `Today: ${todayReceiptsCount.value} order${todayReceiptsCount.value === 1 ? '' : 's'} and ${formatCurrency(todaySales.value)} in completed sales.`
 )
 } else {
 ...
 parts.push(
 `Month to date: ${formatCurrency(monthSales.value)} across ${completedReceiptsCount.value} completed receipt${completedReceiptsCount.value === 1 ? '' : 's'}.`
 )
 }
 if (outstandingCount.value > 0) {
 parts.push(
 `${outstandingCount.value} open balance${outstandingCount.value === 1 ? '' : 'es'} totaling ${formatCurrency(outstandingBalanceTotal.value)} need follow-up.`
 )
 }
 if (lowStockItems.value.length > 0) {
 parts.push(
 `${lowStockItems.value.length} inventory line${lowStockItems.value.length === 1 ? '' : 's'} are at or below your low-stock threshold.`
 )
 }
 if (salesLast7Days.value > 0) {
 parts.push(
 `Last 7 days: ${salesLast7Days.value} completed sale${salesLast7Days.value === 1 ? '' : 's'} (${formatCurrency(revenueLast7Days.value)}).`
 )
 }
 return parts.join(' ')
 })
 ...
 description: `${outstandingCount.value} receipt${outstandingCount.value === 1 ? '' : 's'} with ${formatCurrency(outstandingBalanceTotal.value)} still owed.`,
 ...
 description: `${pendingReceiptsCount.value} receipt${pendingReceiptsCount.value === 1 ? ' is' : 's are'} awaiting completion.`,
 ...
 level: 'warning',
 title: 'Low stock',
 description: `${lowStockItems.value.length} folder or SKU${lowStockItems.value.length === 1 ? '' : 's'} need restocking attention.`,
 ...
 level: 'info',
 title: 'Get started',
 description: 'Create your first receipt to populate sales metrics, customer insights, and revenue trends.',
 href: '/dashboard/receipts',
 cta: 'Create receipt',
 })
 ...
 level: 'info',
 title: 'Quiet week',
 description: 'No completed sales in the last 7 days. Review pricing, stock levels, or follow up with customers.',
