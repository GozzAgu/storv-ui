...
export const usePreferences = () => {
  ...
  const formatCurrency = (
    value: number,
    options?: { showSymbol?: boolean; fromCurrency?: string }
  ) => {
    if (!import.meta.client) {
      ...
      const locale =
        preferences.value.region === 'NG'
          ? 'en-NG'
          : preferences.value.region === 'GB'
          ? 'en-GB'
          : preferences.value.region === 'EU'
          ? 'de-DE'
          : 'en-US'
    ...
    if (sourceCurrency !== targetCurrency) {
      try {
        ...
        convertedValue = currencyConversion.convertCurrencySync(
          value,
          sourceCurrency,
          targetCurrency
        )
